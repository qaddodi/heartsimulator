...
class Pathology{
    ...
    mi(bool){
        var top = this.heart.row-this.heart.row*0.5;
        var bottom = this.heart.row;
        var right = this.heart.col;
        var left = this.heart.col-this.heart.col*0.6;
        if(bool){
            this.MI = true;
            for(var i=0; i<this.heart.cells.length;i++){
                for(var j=0; j<this.heart.cells[i].length;j++){
...
var cellSize = 10;
var rows = (canvas.height-100)/cellSize;
var cols = canvas.width/cellSize;
var heart = new Heart(rows,cols,cellSize);
heart.createCells();
heart.createSeptum();
